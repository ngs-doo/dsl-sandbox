 ...
 * @property \NGS\Timestamp $publishedOn a timestamp with time zone, can be null
 * @property \NGS\Timestamp $createdAt a timestamp with time zone
 * @property \NGS\Timestamp $modifiedAt autogenerated by server (read-only)
 *
 * @package CMS
 * @version 0.9.9 beta
 */
class Post extends \NGS\Patterns\AggregateRoot implements \IteratorAggregate
{
    ...
    /**
     * Constructs object using a key-property array or instance of class "CMS\Post"
     *
     * @param array|void $data key-property array or instance of class "CMS\Post" or pass void to provide all fields with defaults
     */
    ...
    private function fromArray(array $data)
    {
        ...
        unset($data['content']);
        if (array_key_exists('publishedOn', $data))
            $this->setPublishedOn($data['publishedOn']);
        ...
        if (count($data) !== 0 && \NGS\Utils::WarningsAsErrors())
            throw new \InvalidArgumentException('Superflous array keys found in "CMS\Post" constructor: '.implode(', ', array_keys($data)));
    }
...
// ============================================================================
    ...
    public function __get($name)
    {
        ...
        if ($name === 'publishedOn')
            return $this->getPublishedOn(); // a timestamp with time zone, can be null
        if ($name === 'createdAt')
            return $this->getCreatedAt(); // a timestamp with time zone
        if ($name === 'modifiedAt')
            return $this->getModifiedAt(); // a timestamp with time zone

        throw new \InvalidArgumentException('Property "'.$name.'" in class "CMS\Post" does not exist and could not be retrieved!');
    }
...
// ============================================================================
    ...
    public function __isset($name)
    {
        ...
        if ($name === 'content')
            return true; // a string (always set)
        if ($name === 'publishedOn')
            return $this->getPublishedOn() !== null; // a timestamp with time zone, can be null
        if ($name === 'createdAt')
    ...
    private static $_read_only_properties = array('URI', 'ID', 'modifiedAt');
     ...
     * @return \NGS\Timestamp
     */
    private function setModifiedAt($value)
    {
        if ($value === null)
    ...
    public function __set($name, $value)
    {
        if(in_array($name, self::$_read_only_properties, true))
            throw new \LogicException('Property "'.$name.'" in "CMS\Post" cannot be set, because it is read-only!');
        if ($name === 'title')
            return $this->setTitle($value); // a string
        if ($name === 'content')
            return $this->setContent($value); // a string
        if ($name === 'publishedOn')
            return $this->setPublishedOn($value); // a timestamp with time zone, can be null
        if ($name === 'createdAt')
            return $this->setCreatedAt($value); // a timestamp with time zone
        throw new \InvalidArgumentException('Property "'.$name.'" in class "CMS\Post" does not exist and could not be set!');
    }
    ...
    public function __unset($name)
    {
        ...
        if ($name === 'publishedOn')
            $this->setPublishedOn(null);; // a timestamp with time zone, can be null
        if ($name === 'createdAt')
    ...
    /**
     * Create or update CMS\Post instance (server call)
     *
    ...
    private function updateWithAnother(\CMS\Post $result)
    {
        $this->URI = $result->URI;
    ...
    {
        return \CMS\PostJsonConverter::toJson($this);
    }

    public static function fromJson($item)
    {
        return \CMS\PostJsonConverter::fromJson($item);
    }

    public function __toString()
    {
        return 'CMS\Post'.$this->toJson();
    }

    public function __clone()
    {
        return \CMS\PostArrayConverter::fromArray(\CMS\PostArrayConverter::toArray($this));
    }

    public function toArray()
    {
        return \CMS\PostArrayConverter::toArray($this);
    }
    ...
    {
        return new \ArrayIterator(\CMS\PostArrayConverter::toArray($this));
    }
}