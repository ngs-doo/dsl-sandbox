...
class Product extends \NGS\Patterns\AggregateRoot implements \IteratorAggregate
{
    ...
    /**
     * Constructs object using a key-property array or instance of class "Store\Product"
     *
     * @param array|void $data key-property array or instance of class "Store\Product" or pass void to provide all fields with defaults
     */
    ...
    private function fromArray(array $data)
    {
        ...
        if (count($data) !== 0 && \NGS\Utils::WarningsAsErrors())
            throw new \InvalidArgumentException('Superflous array keys found in "Store\Product" constructor: '.implode(', ', array_keys($data)));
    }
...
// ============================================================================
    ...
    public function __get($name)
    {
        ...
        throw new \InvalidArgumentException('Property "'.$name.'" in class "Store\Product" does not exist and could not be retrieved!');
    }
...
// ============================================================================
    ...
    public function __set($name, $value)
    {
        if(in_array($name, self::$_read_only_properties, true))
            throw new \LogicException('Property "'.$name.'" in "Store\Product" cannot be set, because it is read-only!');
        if ($name === 'name')
            return $this->setName($value); // a string
        if ($name === 'price')
            return $this->setPrice($value); // a money amount
        throw new \InvalidArgumentException('Property "'.$name.'" in class "Store\Product" does not exist and could not be set!');
    }
    ...
    /**
     * Create or update Store\Product instance (server call)
     *
    ...
    private function updateWithAnother(\Store\Product $result)
    {
        $this->URI = $result->URI;
    ...
    {
        return \Store\ProductJsonConverter::toJson($this);
    }

    public static function fromJson($item)
    {
        return \Store\ProductJsonConverter::fromJson($item);
    }

    public function __toString()
    {
        return 'Store\Product'.$this->toJson();
    }

    public function __clone()
    {
        return \Store\ProductArrayConverter::fromArray(\Store\ProductArrayConverter::toArray($this));
    }

    public function toArray()
    {
        return \Store\ProductArrayConverter::toArray($this);
    }
    ...
    {
        return new \ArrayIterator(\Store\ProductArrayConverter::toArray($this));
    }

    /**
     * Find data using declared specification findByPriceRange
     ...
     */
    public static function findByPriceRange($min, $max, $searchLimit = null, $searchOffset = null)
    {
        require_once __DIR__.'/Product/findByPriceRange.php';
        $specification = new \Store\Product\findByPriceRange(array('min' => $min, 'max' => $max));
