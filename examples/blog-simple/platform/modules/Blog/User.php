...
class User extends \NGS\Patterns\AggregateRoot implements \IteratorAggregate
{
    ...
    /**
     * Constructs object using a key-property array or instance of class "Blog\User"
     *
     * @param array|void $data key-property array or instance of class "Blog\User" or pass void to provide all fields with defaults
     */
    ...
    private function fromArray(array $data)
    {
        ...
        if (count($data) !== 0 && \NGS\Utils::WarningsAsErrors())
            throw new \InvalidArgumentException('Superflous array keys found in "Blog\User" constructor: '.implode(', ', array_keys($data)));
    }
...
// ============================================================================
    ...
    public function __get($name)
    {
        ...
        throw new \InvalidArgumentException('Property "'.$name.'" in class "Blog\User" does not exist and could not be retrieved!');
    }
...
// ============================================================================
    ...
    public function __set($name, $value)
    {
        if(in_array($name, self::$_read_only_properties, true))
            throw new \LogicException('Property "'.$name.'" in "Blog\User" cannot be set, because it is read-only!');
        if ($name === 'email')
            return $this->setEmail($value); // a string
        if ($name === 'comments')
            return $this->setComments($value); // an array of objects of class "Blog\Comment"
        throw new \InvalidArgumentException('Property "'.$name.'" in class "Blog\User" does not exist and could not be set!');
    }
    ...
    /**
     * Create or update Blog\User instance (server call)
     *
    ...
    private function updateWithAnother(\Blog\User $result)
    {
        $this->URI = $result->URI;
    ...
    {
        return \Blog\UserJsonConverter::toJson($this);
    }

    public static function fromJson($item)
    {
        return \Blog\UserJsonConverter::fromJson($item);
    }

    public function __toString()
    {
        return 'Blog\User'.$this->toJson();
    }

    public function __clone()
    {
        return \Blog\UserArrayConverter::fromArray(\Blog\UserArrayConverter::toArray($this));
    }

    public function toArray()
    {
        return \Blog\UserArrayConverter::toArray($this);
    }
    ...
    {
        return new \ArrayIterator(\Blog\UserArrayConverter::toArray($this));
    }
}