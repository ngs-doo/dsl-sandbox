...
class Product extends \NGS\Patterns\AggregateRoot implements \IteratorAggregate
{
    ...
    /**
     * Constructs object using a key-property array or instance of class "Shop\Product"
     *
     * @param array|void $data key-property array or instance of class "Shop\Product" or pass void to provide all fields with defaults
     */
    ...
    private function fromArray(array $data)
    {
        ...
        if (count($data) !== 0 && \NGS\Utils::WarningsAsErrors())
            throw new \InvalidArgumentException('Superflous array keys found in "Shop\Product" constructor: '.implode(', ', array_keys($data)));
    }
...
// ============================================================================
    ...
    public function __get($name)
    {
        ...
        throw new \InvalidArgumentException('Property "'.$name.'" in class "Shop\Product" does not exist and could not be retrieved!');
    }
...
// ============================================================================
    ...
    public function __set($name, $value)
    {
        if(in_array($name, self::$_read_only_properties, true))
            throw new \LogicException('Property "'.$name.'" in "Shop\Product" cannot be set, because it is read-only!');
        if ($name === 'name')
            return $this->setName($value); // a string
        if ($name === 'price')
            return $this->setPrice($value); // a money amount
        throw new \InvalidArgumentException('Property "'.$name.'" in class "Shop\Product" does not exist and could not be set!');
    }
    ...
    /**
     * Create or update Shop\Product instance (server call)
     *
    ...
    private function updateWithAnother(\Shop\Product $result)
    {
        $this->URI = $result->URI;
    ...
    {
        return \Shop\ProductJsonConverter::toJson($this);
    }

    public static function fromJson($item)
    {
        return \Shop\ProductJsonConverter::fromJson($item);
    }

    public function __toString()
    {
        return 'Shop\Product'.$this->toJson();
    }

    public function __clone()
    {
        return \Shop\ProductArrayConverter::fromArray(\Shop\ProductArrayConverter::toArray($this));
    }

    public function toArray()
    {
        return \Shop\ProductArrayConverter::toArray($this);
    }
    ...
    {
        return new \ArrayIterator(\Shop\ProductArrayConverter::toArray($this));
    }
}